...
export const useCanvasStore = create(
  persist(
    (set, get) => ({
      ...
      // "Portfolio" page (see STALE_DEFAULT_IMAGE_URLS above). If every
      // image element on the page still holds a known auto-generated URL —
      // i.e. the admin has never touched it — the whole elements array is
      // replaced with the current DEFAULT_PAGE_ELEMENTS. A field-by-field
      // id match (as a prior version of this did) can't work here: the
      // latest regeneration inserted cover slides and a 4th column, so old
      // ids no longer line up with the same content at the same id. Any
      // page with even one non-matching image — a genuine admin edit — is
      // left completely untouched.
      refreshStaleDefaultImages() {
        set((s) => ({
          pages: s.pages.map((p) => {
            if (p.id !== "default") return p;
            const imageEls = p.elements.filter((el) => el.type === "image");
            const allPristine =
              imageEls.length > 0 &&
              imageEls.every((el) => (STALE_DEFAULT_IMAGE_URLS[el.id] || []).includes(el.src));
            return allPristine ? { ...p, elements: DEFAULT_PAGE_ELEMENTS } : p;
          }),
        }));
      },
